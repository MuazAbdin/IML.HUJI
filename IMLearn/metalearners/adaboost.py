...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.models_ = np.array([None] * self.iterations_)
        self.weights_ = np.array([None] * self.iterations_)

        for i in range(self.iterations_):
            self.models_[i] = self.wl_().fit(X, y * self.D_)
            predicted = self.models_[i].predict(X)
            epsilon = np.sum((np.abs(predicted - y) / 2) * self.D_)
            self.weights_[i] = 0.5 * np.log(1.0 / epsilon - 1)
            self.D_ *= np.exp((-1) * self.weights_[i] * y * predicted)
